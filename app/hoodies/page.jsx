...
const Hoodies = async () => {
  ...
  return (
    <>
      {Object.values(Data.hoodies).length === 0 && (
        <p className="text-center m-20">
          Sorry! Currently Hoodies are out of stock, We are adding soon, so stay
          tuned.
        </p>
      )}
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
        {Object.values(Data.hoodies) &&
          Object.values(Data.hoodies).map((item) => (
            <div key={item._id}>
