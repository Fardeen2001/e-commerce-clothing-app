...
const Stickers = async () => {
  ...
  return (
    <>
      {Object.values(Data.stickers).length === 0 && (
        <p className="text-center m-20">
          Sorry! Currently Stickers are out of stock, We are adding soon, so
          stay tuned.
        </p>
      )}
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
        {Object.values(Data.stickers) &&
          Object.values(Data.stickers).map((item) => (
            <div key={item._id}>
